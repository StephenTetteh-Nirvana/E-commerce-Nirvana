    ...
    <div class="container">
           <div class="head">
             ...
             <div class="mother-box">
                <div v-for="men in mens" :key="men.id">
                <div class="product-box">
                    <img :src="require('../images/' + men.image)" alt="Product Image" />
                    <div class="box-one">
                        <h3>{{ men.name }}</h3>
                        <h2>${{ men.price }}.00</h2>
                        <button class="btn-one" @click="addToCart(men)">Add To Cart</button>
                    </div>
    ...
    setup() {
        ...
        const updateFail = ref(false)
            ...
            const loadProducts = async () => {
                try {
                    const productsCollection = collection(db, 'Men Clothing');
                    const snapshot = await getDocs(productsCollection);
                    mens.value = snapshot.docs.map((doc) => ({
                    id: doc.id,
                    name:doc.data().Name,
