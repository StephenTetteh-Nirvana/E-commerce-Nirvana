    ...
    <div class="container">
           <div class="head">
             ...
             <div class="mother-box">
                <div v-for="woman in women" :key="woman.id">
                <div class="product-box">
                    <img :src="require('../images/' + woman.image)" alt="Product Image" />
                    <div class="box-one">
                        <h3>{{ woman.name }}</h3>
                        <h2>${{ woman.price }}.00</h2>
                        <button class="btn-one" @click="addToCart(woman)">Add To Cart</button>
                    </div>
    ...
    setup() {
        ...
        const updateFail = ref(false)
            ...
            const loadProducts = async () => {
                try {
                    const productsCollection = collection(db, 'Women Clothing');
                    const snapshot = await getDocs(productsCollection);
                    women.value = snapshot.docs.map((doc) => ({
                    id: doc.id,
                    name:doc.data().Name,
